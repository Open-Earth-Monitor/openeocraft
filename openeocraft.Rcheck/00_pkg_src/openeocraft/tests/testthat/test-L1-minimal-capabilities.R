...
is_valid_endpoint <- function(endpoint, implemented_endpoints) {
    ...
    if (is.null(path) || is.null(methods)) {
        return(FALSE)
    }
    for (method in methods) {
        if (!method %in% implemented_endpoints[[path]]) {
            return(FALSE)
        }
    }
    TRUE
...
test_that(paste0(
    "GET /.well-known/openeo: Valid response with at ",
    "least url and api_version per instance"
), {
    expect_true(
        !is.null(mock_well_known_response$url),
        "URL is missing in /.well-known/openeo response"
    )
    expect_true(
        !is.null(mock_well_known_response$api_version),
        "API version is missing in /.well-known/openeo response"
    )
})

# Test: Root endpoint basic structure
test_that(paste0(
    "GET /: Valid response with at least api/backend/stac_version, ",
    "id, title, description, endpoints, and links"
), {
    expect_true(
        !is.null(mock_root_response$api_version),
        "API version is missing in / response"
    )
    expect_true(
        !is.null(mock_root_response$backend_version),
        "Backend version is missing in / response"
    )
    expect_true(
        !is.null(mock_root_response$stac_version),
        "STAC version is missing in / response"
    )
    expect_true(
        !is.null(mock_root_response$id),
        "ID is missing in / response"
    )
    expect_true(
        !is.null(mock_root_response$title),
        "Title is missing in / response"
    )
    expect_true(
        !is.null(mock_root_response$description),
        "Description is missing in / response"
    )
    expect_true(
        length(mock_root_response$endpoints) > 0,
        "Endpoints are missing in / response"
    )
    expect_true(
        length(mock_root_response$links) > 0, "Links are missing in / response"
    )
})

# Test: Endpoint listing completeness
test_that(paste0(
    "GET / > endpoints: All implemented endpoints are ",
    "listed with their methods"
), {
...
test_that(paste0(
    "GET / > endpoints: No endpoints are listed ",
    "that are not implemented or faulty"
), {
    implemented_endpoints <- list(
        "/collections" = c("GET"),
        "/processes" = c("GET"),
